require "test_helper"
require "net/http"
require "down/httpx"
require "httpx"
require "json"

describe Down::Httpx do
  describe "#initialize" do
    it "accepts a hash for overriding options" do
      down = Down::Httpx.new(headers: { "Foo" => "Bar" })
      tempfile = down.download("#{$httpbin}/headers")
      headers = JSON.parse(tempfile.read)#["headers"]
      assert_equal "Bar",                   headers["Foo"]
      assert_equal "Down/#{Down::VERSION}", headers["User-Agent"]
    end

    it "accepts a block for overriding options" do
      down = Down::Httpx.new { |client| client.headers("Foo" => "Bar") }
      tempfile = down.download("#{$httpbin}/headers")
      headers = JSON.parse(tempfile.read)#["headers"]
      assert_equal "Bar",                   headers["Foo"]
      assert_equal "Down/#{Down::VERSION}", headers["User-Agent"]
    end

    it "accepts request method" do
      [:post, "POST"].each do |method|
        down = Down::Httpx.new(method: method)

        tempfile = down.download("#{$httpbin}/post")
        assert_equal "Down/#{Down::VERSION}", JSON.parse(tempfile.read)["headers"]["User-Agent"]
      end
    end
  end

  it "downloads content from url" do
    tempfile = Down::Httpx.download("#{$httpbin}/bytes/100?seed=0")
    assert_equal Net::HTTP.get(URI("#{$httpbin}/bytes/100?seed=0")).to_s, tempfile.read
  end

  it "opens the tempfile in binary mode" do
    tempfile = Down::Httpx.download("#{$httpbin}/bytes/100?seed=0")
    assert tempfile.binmode?
  end

  it "accepts maximum size" do
    assert_raises(Down::TooLarge) do
      Down::Httpx.download("#{$httpbin}/bytes/10", max_size: 5)
    end

    assert_raises(Down::TooLarge) do
      Down::Httpx.download("#{$httpbin}/stream-bytes/10", max_size: 5)
    end

    tempfile = Down::Httpx.download("#{$httpbin}/bytes/10", max_size: 10)
    assert File.exist?(tempfile.path)

    tempfile = Down::Httpx.download("#{$httpbin}/stream-bytes/10", max_size: 15)
    assert File.exist?(tempfile.path)
  end

  it "infers file extension from url" do
    tempfile = Down::Httpx.download("#{$httpbin}/robots.txt")
    assert_equal ".txt", File.extname(tempfile.path)

    tempfile = Down::Httpx.download("#{$httpbin}/robots.txt?foo=bar")
    assert_equal ".txt", File.extname(tempfile.path)

    tempfile = Down::Httpx.download("#{$httpbin}/redirect-to?url=#{$httpbin}/robots.txt")
    assert_equal ".txt", File.extname(tempfile.path)
  end

  it "accepts :content_length_proc" do
    tempfile = Down::Httpx.download("#{$httpbin}/stream-bytes/100", content_length_proc: -> (length) { @length = length })
    refute instance_variable_defined?(:@length)

    tempfile = Down::Httpx.download("#{$httpbin}/bytes/100", content_length_proc: -> (length) { @length = length })
    assert_equal 100, @length
  end

  it "accepts :progress_proc" do
    tempfile = Down::Httpx.download("#{$httpbin}/stream-bytes/100?chunk_size=10", progress_proc: -> (progress) { (@progress ||= []) << progress })
    assert_equal 100, @progress.last
  end

  it "accepts HTTPX.rb options" do
    tempfile = Down::Httpx.download("#{$httpbin}/user-agent", headers: {"User-Agent" => "Janko"})
    assert_equal "Janko", JSON.parse(tempfile.read)["user-agent"]
  end

  it "adds #headers and #url" do
    tempfile = Down::Httpx.download("#{$httpbin}/response-headers?Foo=Bar")
    assert_equal "Bar",                                  tempfile.headers["Foo"]
    assert_equal "#{$httpbin}/response-headers?Foo=Bar", tempfile.url

    tempfile = Down::Httpx.download("#{$httpbin}/redirect-to?url=#{$httpbin}/response-headers?Foo=Bar")
    assert_equal "Bar",                                  tempfile.headers["Foo"]
    assert_equal "#{$httpbin}/response-headers?Foo=Bar", tempfile.url
  end

  it "adds #original_filename extracted from Content-Disposition" do
    tempfile = Down::Httpx.download("#{$httpbin}/response-headers?Content-Disposition=inline;%20filename=\"my%20filename.ext\"")
    assert_equal "my filename.ext", tempfile.original_filename

    tempfile = Down::Httpx.download("#{$httpbin}/response-headers?Content-Disposition=inline;%20filename=\"my%2520filename.ext\"")
    assert_equal "my filename.ext", tempfile.original_filename

    tempfile = Down::Httpx.download("#{$httpbin}/response-headers?Content-Disposition=inline;%20filename=myfilename.ext%20")
    assert_equal "myfilename.ext", tempfile.original_filename
  end

  it "adds #original_filename extracted from URI path if Content-Disposition is blank" do
    tempfile = Down::Httpx.download("#{$httpbin}/robots.txt")
    assert_equal "robots.txt", tempfile.original_filename

    tempfile = Down::Httpx.download("#{$httpbin}/basic-auth/user/pass%20word") do |client|
      client.basic_auth("user", "pass word")
    end
    assert_equal "pass word", tempfile.original_filename

    tempfile = Down::Httpx.download("#{$httpbin}/response-headers?Content-Disposition=inline;%20filename=")
    assert_equal "response-headers", tempfile.original_filename

    tempfile = Down::Httpx.download("#{$httpbin}/response-headers?Content-Disposition=inline;%20filename=\"\"")
    assert_equal "response-headers", tempfile.original_filename

    tempfile = Down::Httpx.download("#{$httpbin}/")
    assert_nil tempfile.original_filename

    tempfile = Down::Httpx.download("#{$httpbin}")
    assert_nil tempfile.original_filename
  end

  it "adds #content_type extracted from Content-Type" do
    tempfile = Down::Httpx.download("#{$httpbin}/image/png")
    assert_equal "image/png", tempfile.content_type

    tempfile = Down::Httpx.download("#{$httpbin}/encoding/utf8")
    assert_equal "text/html; charset=utf-8", tempfile.headers["Content-Type"]
    assert_equal "text/html", tempfile.content_type
  end

  it "adds #charset extracted from Content-Type" do
    tempfile = Down::Httpx.download("#{$httpbin}/get")
    tempfile.headers["Content-Type"] = "text/plain; charset=utf-8"
    assert_equal "utf-8", tempfile.charset

    tempfile.headers.delete("Content-Type")
    assert_nil tempfile.charset

    tempfile.headers["Content-Type"] = nil
    assert_nil tempfile.charset

    tempfile.headers["Content-Type"] = ""
    assert_nil tempfile.charset
  end

  it "accepts download destination" do
    tempfile = Tempfile.new("destination")
    result = Down::Httpx.download("#{$httpbin}/bytes/#{20*1024}?seed=0", destination: tempfile.path)
    assert_equal Net::HTTP.get(URI("#{$httpbin}/bytes/#{20*1024}?seed=0")).to_s, File.binread(tempfile.path)
    assert_nil result
  end

  it "accepts request method" do
    [:post, "POST"].each do |method|
      tempfile = Down::Httpx.download("#{$httpbin}/post", method: method)
      assert_equal "Down/#{Down::VERSION}", JSON.parse(tempfile.read)["headers"]["User-Agent"]
    end
  end

  it "follows redirects" do
    io = Down::Httpx.download("#{$httpbin}/redirect/1")
    assert_equal "#{$httpbin}/get", JSON.parse(io.read)["url"]
    io = Down::Httpx.download("#{$httpbin}/redirect/2")
    assert_equal "#{$httpbin}/get", JSON.parse(io.read)["url"]
    assert_raises(Down::TooManyRedirects) { Down::Httpx.download("#{$httpbin}/redirect/3") }

    io = Down::Httpx.download("#{$httpbin}/redirect/3", max_redirects: 3)
    assert_equal "#{$httpbin}/get", JSON.parse(io.read)["url"]
    assert_raises(Down::TooManyRedirects) { Down::Httpx.download("#{$httpbin}/redirect/4", max_redirects: 3) }

    io = Down::Httpx.download("#{$httpbin}/absolute-redirect/1")
    assert_equal "#{$httpbin}/get", JSON.parse(io.read)["url"]
    io = Down::Httpx.download("#{$httpbin}/relative-redirect/1")
    assert_equal "#{$httpbin}/get", JSON.parse(io.read)["url"]
  end

  it "returns content in encoding specified by charset" do
    assert_equal Encoding::BINARY, Down::Httpx.download("#{$httpbin}/stream/10").to_s.encoding

    assert_equal Encoding::BINARY, Down::Httpx.download("#{$httpbin}/get").to_s.encoding

    assert_equal Encoding::UTF_8, Down::Httpx.download("#{$httpbin}/encoding/utf8").to_s.encoding
  end

  it "detects and applies basic authentication from URL" do
    tempfile = Down::Httpx.download("#{$httpbin.sub("http://", '\0user:password@')}/basic-auth/user/password")
    assert_equal true, JSON.parse(tempfile.to_s)["authenticated"]
  end

  it "raises on Httpx error responses" do
    error = assert_raises(Down::ClientError) { Down::Httpx.download("#{$httpbin}/status/404") }
    assert_equal "404 Not Found", error.message
    assert_kind_of HTTPX::Response, error.response

    error = assert_raises(Down::ServerError) { Down::Httpx.download("#{$httpbin}/status/500") }
    assert_equal "500 Internal Server Error", error.message
    assert_kind_of HTTPX::Response, error.response
  end

  it "re-raises invalid URL errors" do
    scheme_error = assert_raises { Down::Httpx.download("foo://example.org") }
    assert_match(/unsupported URI scheme/, scheme_error.message)
    assert_raises(Down::InvalidUrl) { Down::Httpx.download("http://example.org\\foo") }
  end

  it "re-raises connection errors" do
    assert_raises(Down::ConnectionError) { Down::Httpx.download("http://localhost:99999") }
  end

  it "re-raises timeout errors" do
    assert_raises(Down::TimeoutError) { Down::Httpx.download("#{$httpbin}/delay/0.5"){ |c| c.timeout(loop_timeout: 0)}.read }
  end

  it "re-raises SSL errors" do
    HTTPX::TCP.any_instance.expects(:connect).raises(OpenSSL::SSL::SSLError)

    assert_raises(Down::SSLError) { Down::Httpx.download($httpbin) }
  end
end
